...
vows.describe('node-ical').addBatch({
  'when parsing test1.ics (node conferences schedule from lanyrd.com, modified)': {
        topic: () => ical.parseFile('./test/test1.ics')
    ...
    ,'event 47f6e' : {
      topic: events => _.select(_.values(events),
          x => x.uid ==='47f6ea3f28af2986a2192fa39a91fa7d60d26b76')[0]
    ...
    , 'event 480a' : {
      topic: events => _.select(_.values(events),
          x => x.uid ==='480a3ad48af5ed8965241f14920f90524f533c18')[0]
    ...
    , 'event d4c8' :{
      topic : events => _.select(_.values(events),
            x => x.uid === 'd4c826dfb701f611416d69b4df81caf9ff80b03a')[0]
  ...
  , 'with test2.ics (testing ical features)' : {
    topic: () => ical.parseFile('./test/test2.ics')
    , 'todo item uid4@host1.com' : {
      topic : items => items['uid4@host1.com']
      , 'is a VTODO' : topic => {
        assert.equal(topic.type, 'VTODO')
      }
    }
    , 'vfreebusy' : {
      topic: events => _.select(_.values(events), x => x.type === 'VFREEBUSY')[0]
      , 'has a URL' : topic => {
        assert.equal(topic.url, 'http://www.host.com/calendar/busytime/jsmith.ifb');
      }
    }
    , 'vfreebusy first freebusy' : {
      topic: events => _.select(_.values(events), x => x.type === 'VFREEBUSY')[0].freebusy[0]
  ...
  , 'with test3.ics (testing tvcountdown.com)' : {
    topic: () => ical.parseFile('./test/test3.ics')
    , 'event -83' : {
      topic: events => _.select(_.values(events), x => x.uid === '20110505T220000Z-83@tvcountdown.com')[0]
      , 'has a start datetime' : topic => {
  ...
  , 'with test4.ics (testing tripit.com)' : {
    topic: () => ical.parseFile('./test/test4.ics')
    , 'event c32a5...' : {
      topic: events => _.select(_.values(events), x => x.uid === 'c32a5eaba2354bb29e012ec18da827db90550a3b@tripit.com')[0]
      , 'has a start datetime' : topic => {
        assert.equal(topic.start.getFullYear(), 2011);
  ...
  , 'with test5.ics (testing meetup.com)' : {
     topic: () => ical.parseFile('./test/test5.ics')
    , 'event nsmxnyppbfc@meetup.com' : {
      topic: events => _.select(_.values(events), x => x.uid === 'event_nsmxnyppbfc@meetup.com')[0]
      , 'has a start' : topic => {
  ...
  , 'with test6.ics (testing assembly.org)' : {
     topic: () => ical.parseFile('./test/test6.ics')
    , 'event with no ID' : {
      topic: events => _.select(_.values(events), x => x.summary === 'foobar Summer 2011 starts!')[0]
      , 'has a start' : topic => {
        assert.equal(topic.start.toISOString(), new Date(2011, 7, 4, 12, 0,0).toISOString())
      }
    }
  , 'event with rrule' :{
      topic: events => _.select(_.values(events), x => x.summary == "foobarTV broadcast starts")[0]
      , "Has an RRULE": topic => {
   ...
   , 'with test7.ics (testing dtstart of rrule)' :{
    topic: () => ical.parseFile('./test/test7.ics'),
  ...
  , "with test 8.ics (VTODO completion)": {
    topic: () => ical.parseFile('./test/test8.ics'),
    'grabbing VTODO task': {
        topic: topic => _.values(topic)[0],
        'task completed': task => {
  ...
  , "with test 9.ics (VEVENT with VALARM)": {
    topic: () => ical.parseFile('./test/test9.ics'),
  ...
  , 'with test 11.ics (VEVENT with custom properties)': {
      topic: () => ical.parseFile('./test10.ics'),
  ...
  'with test10.ics': {
    topic: () => ical.parseFile('./test/test10.ics'),
  ...
  'with test11.ics (testing zimbra freebusy)': {
    topic: () => ical.parseFile('./test/test11.ics'),

    'freebusy params' : {
      topic: events => _.values(events)[0]
      , 'has a URL' : topic => {
    ...
    , 'freebusy busy events' : {
      topic: events => _.select(_.values(events)[0].freebusy, x => x.type === 'BUSY')[0]
      , 'has an start datetime' : topic => {
        assert.equal(topic.start.getFullYear(), 2014);
        assert.equal(topic.start.getMonth(), 3);
